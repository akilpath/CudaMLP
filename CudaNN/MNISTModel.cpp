...
void MNISTModel::train() {
	...
	for (int i = 0; i < epochs; i++) {
		...
		for (int batch_index = 0; batch_index * batch_size < mnist.training_images.size(); batch_index++) {
			...
			relu_2->backward(gradients, gradients);
			hidden_layer->backward(temploss_1, gradients);
			//printf("Temploss 1\n");
			...
			//printf("Temploss 2\n");
			//temploss_1.print_data();
			input_layer->backward(temploss_2, temploss_1);

			delete data;
			delete labels;
			//printf("Done looping\n");
		}
		epoch_loss /= std::ceil(mnist.training_images.size() / batch_size);
		epoch_accuracy /= std::ceil(mnist.training_images.size() / batch_size);
		printf("Average epoch loss: %f, Average accuracy: %f\n", epoch_loss, epoch_accuracy);
		...
		test_loss /= std::ceil(mnist.test_images.size() / batch_size);
		test_accuracy /= std::ceil(mnist.test_images.size() / batch_size);
